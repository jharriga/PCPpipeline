...
# Define Workload
num_iterations=3    # Number of test run iterations
iteration_pause=6
runtime=10         # Runtime duration for each workload test iteration
...
# TIMER (ms) to measure stop time
read up rest </proc/uptime; prestop="${up%.*}${up#*.}"
pcp_stop
# End TIMER  for 'Stop' Request
read up rest </proc/uptime; poststop="${up%.*}${up#*.}"
duration_stop=$(( 10*(poststop - prestop) ))
echo; echo "> STARTtimer=${duration_start}ms   STOPtimer=${duration_stop}ms"
